...
* Antes de llamar al backend, se debe verificar localmente si el proyecto tiene tareas asignadas recorriendo la señal de tareas. Si tiene tareas, cancelar la operación silenciosamente y mostrar un `alert()` nativo en el navegador con el mensaje: *"No se puede eliminar el proyecto porque aún tiene tareas asignadas."
...
* **Dado** que se muestra un cartel de acción (`actionMessage`), 
